      ...
      Padding(padding: const EdgeInsets.fromLTRB(20, 28, 20, 0), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        SizedBox(height: 40, child: ListView(scrollDirection: Axis.horizontal, physics: const BouncingScrollPhysics(),
          children: MockDataService.divisions.map((d) => Padding(padding: const EdgeInsets.only(right: 8),
            child: FilterChip(
              label: Text(d, style: GoogleFonts.poppins(color: _selectedDiv == d ? Colors.white : (isDark ? const Color(0xFF94A3B8) : const Color(0xFF64748B)), fontSize: 13, fontWeight: FontWeight.w500)),
              selected: _selectedDiv == d, onSelected: (_) => setState(() => _selectedDiv = d),
              selectedColor: const Color(0xFF06B6D4), 
              backgroundColor: isDark ? const Color(0xFF0F172A).withOpacity(0.4) : Colors.white.withOpacity(0.6),
              side: BorderSide(color: _selectedDiv == d ? const Color(0xFF06B6D4) : (isDark ? Colors.white.withOpacity(0.1) : const Color(0xFFE2E8F0))),
